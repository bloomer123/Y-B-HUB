-- Start cache scan immediately, even during loading screen
task.spawn(function()
    repeat task.wait(0.5) until game:IsLoaded()
end)
if not game:IsLoaded() then game.Loaded:Wait() end

-- FPS Unlocker + Lighting Optimizer
pcall(function()
    pcall(function() setfpscap(0) end)
pcall(function() syn.set_fps_cap(0) end)
pcall(function() fluxus.set_fps_cap(0) end)
...
pcall(function()
    -- Disable all particle emitters and beams globally
...
-- Play a sound when the script loads
task.spawn(function()
    pcall(function()
        local loadSound = Instance.new("Sound")
        loadSound.Name = "XiLoadSound"
        loadSound.SoundId = "rbxassetid://8343056948"
        loadSound.Volume = 1
        ...
        -- Failsafe cleanup in case Ended doesn't fire
        task.delay(15, function()
            if loadSound and loadSound.Parent then
                loadSound:Destroy()
            end
        end)
    end)
...
if not SharedState.__AutoTPOnLoadInit then
    ...
    task.spawn(function()
        ...
        -- Fire as soon as data is ready, no char wait
        if not game:IsLoaded() then game.Loaded:Wait() end
        local t = 0
        while t < 500 do
        ...
        -- Wait briefly so 3f script can register its target gen
        task.wait(0.05)
        if _G._3fTargetGen and SharedState.AllAnimalsCache then
            -- Find best 1f/2f gen
            local best1f = 0
            for _, a in ipairs(SharedState.AllAnimalsCache) do
                local _p = findAdorneeGlobal and findAdorneeGlobal(a)
                if a and a.owner ~= LocalPlayer.Name and (not _p or _p.Position.Y <= 25) then
                    if (a.genValue or 0) > best1f then best1f = a.genValue end
                end
            end
            if _G._3fTargetGen > best1f then return end
        end
        -- Wait for character to fully spawn before TPing
        local _cw = 0
        while _cw < 200 do
            ...
            if _h and _hm and _hm.Health > 0 then break end
            task.wait(0.05)
            _cw += 1
        end
        runAutoSnipe()
...
local function instantClone()
    ...
    _G.isCloning = true
    local _cloneStart = tick()
    local _didFire = false
    local _failReason = nil

    pcall(function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if not (char and hum) then _failReason = "no char/hum" return end

        local cloner = LocalPlayer.Backpack:FindFirstChild("Quantum Cloner")
            or char:FindFirstChild("Quantum Cloner")
        if not cloner then _failReason = "no cloner tool" return end
        ...
        if not tpButton then _failReason = "no TeleportToClone button" return end
    ...
    task.wait(0.15) -- give the teleport time to take effect server-side before we return

    _G.isCloning = false
    if _didFire then
        print(string.format("[CLONE] fired after %.3fs", tick()-_cloneStart))
    else
        print(string.format("[CLONE] >>> FAILED after %.3fs reason=%s", tick()-_cloneStart, tostring(_failReason)))
    end
end
...
local Theme = {
    Background      = Color3.fromRGB(8, 10, 18),       -- deeper, slightly cooler black for glass effect
    Surface         = Color3.fromRGB(14, 16, 28),      -- frosted layer
    SurfaceHighlight= Color3.fromRGB(28, 32, 52),      -- elevated frosted highlight
    Accent1         = Color3.fromRGB(80, 255, 140),    -- brighter neon green (kept green family)
    Accent2         = Color3.fromRGB(40, 200, 100),    -- secondary green glow
    TextPrimary     = Color3.fromRGB(245, 250, 255),
    TextSecondary   = Color3.fromRGB(160, 175, 200),   -- cooler secondary text
...
-- Applies frosted-glass look to all existing UI frames automatically.
-- Runs continuously so newly-created frames also get the treatment.
-- ═══════════════════════════════════════════════════════════════
task.spawn(function()
    pcall(function()
        local PG = LocalPlayer:WaitForChild("PlayerGui")

        -- Marker attribute so we don't re-process the same frame
        local APPLIED = "_XiGlassApplied"
        ...
        local function applyGlass(frame)
            ...
            -- Skip tiny elements (knobs, sliders, fills, progress bars)
            local sz = frame.AbsoluteSize
            if sz.X > 0 and sz.X < 40 and sz.Y > 0 and sz.Y < 40 then return end

            -- Frosted background: lower the opacity slightly for the "glass" feel
            -- (only nudge frames that have a solid bg, leave transparent ones alone)
            if frame.BackgroundTransparency < 0.05 then
                local bg = frame.BackgroundColor3
                -- Darken & cool-tone slightly to look like frosted glass
                frame.BackgroundColor3 = Color3.new(
                    math.max(0, bg.R * 0.92),
                    math.max(0, bg.G * 0.94),
                    math.min(1, bg.B * 1.05)
                )
                frame.BackgroundTransparency = 0.18
            end

            -- Smooth corners (only for larger frames, keep buttons crisp)
            if sz.X > 60 and sz.Y > 30 then
                local hasCorner = frame:FindFirstChildOfClass("UICorner")
                if not hasCorner then
                    local c = Instance.new("UICorner")
                    c.CornerRadius = UDim.new(0, 12)
                    c.Parent = frame
                else
                    -- Upgrade existing corners to a softer radius
                    local cur = hasCorner.CornerRadius
                    if cur.Offset > 0 and cur.Offset < 12 then
                        hasCorner.CornerRadius = UDim.new(0, 12)
                    end
                end
            end

            -- Subtle vertical gradient for that "frosted" depth
            if sz.X > 80 and sz.Y > 50 and not frame:FindFirstChildOfClass("UIGradient") then
            ...
            -- Soft neon stroke (glassmorphism edge highlight)
            -- Only soften EXISTING strokes - don't add new ones (avoids border lines everywhere)
            if sz.X > 60 and sz.Y > 30 then
                local existing = frame:FindFirstChildOfClass("UIStroke")
                if existing then
                    -- Soften any existing harsh strokes
                    if existing.Thickness > 2 then existing.Thickness = 1.2 end
                    if existing.Transparency < 0.3 then existing.Transparency = 0.4 end
                end
            end
        end
        ...
        local function scanGui(gui)
            ...
            for _, d in ipairs(gui:GetDescendants()) do
                if d:IsA("Frame") or d:IsA("TextButton") or d:IsA("ImageButton") or d:IsA("ScrollingFrame") then
                    pcall(applyGlass, d)
                end
            end
        end

        -- Initial pass: re-scan multiple times as the script builds UI over the first few seconds
        for i = 1, 15 do
        ...
        -- Continuous: catch any UI created later
        PG.DescendantAdded:Connect(function(d)
            task.wait(0.1) -- let the script set its props first
            if d:IsA("Frame") or d:IsA("TextButton") or d:IsA("ImageButton") or d:IsA("ScrollingFrame") then
                pcall(applyGlass, d)
            end
        end)
...
-- Load the priority list from the saved Config if present and valid, otherwise use defaults.
local PRIORITY_LIST
if Config and type(Config.PriorityList) == "table" and #Config.PriorityList > 0 then
    PRIORITY_LIST = {}
    local seen = {}
    for _, name in ipairs(Config.PriorityList) do
        if type(name) == "string" and name ~= "" and not seen[name:lower()] then
            seen[name:lower()] = true
            table.insert(PRIORITY_LIST, name)
        end
    end
    if #PRIORITY_LIST == 0 then
        PRIORITY_LIST = table.clone and table.clone(DEFAULT_PRIORITY_LIST) or {table.unpack(DEFAULT_PRIORITY_LIST)}
    end
else
    PRIORITY_LIST = table.clone and table.clone(DEFAULT_PRIORITY_LIST) or {table.unpack(DEFAULT_PRIORITY_LIST)}
end

-- Helper: persist PRIORITY_LIST into Config and write to disk.
local function SavePriorityList()
    if Config then
        Config.PriorityList = {}
        for i, name in ipairs(PRIORITY_LIST) do
            Config.PriorityList[i] = name
        end
...
task.spawn(function()
    ...
    titleLabel.BackgroundTransparency = 1; titleLabel.Text = "YOU & BLOOM HUB"
...
task.spawn(function()
    ...
    local function getAdminPanelSortKey(plr)
        ...
        if brainrotName then
            local brLower = tostring(brainrotName):lower()
            for i, pName in ipairs(PRIORITY_LIST) do
                if pName:lower() == brLower then
...
local CLONE_POSITIONS_FLOOR = {
    [1]=Vector3.new(-476, -4, 221), [5]=Vector3.new(-342, -4, 220),
    [2]=Vector3.new(-476, -4, 114), [6]=Vector3.new(-342, -4, 114),
    [3]=Vector3.new(-476, -4, 7),   [7]=Vector3.new(-342, -4, 6),
    [4]=Vector3.new(-476, -4, -100),[8]=Vector3.new(-342, -4, -100)
}
...
-- tp.lua 2f (haze V1 front wall for 3rd floor only)
...
local function getClosestBaseSign(brainrotPart)
    ...
    local pos = brainrotPart:IsA("BasePart") and brainrotPart.Position or brainrotPart
...
local function getBestTeleportV2SecondFloorPoint(medPoint, brainrotPos)
    ...
    local bestPoint, bestMedDist, bestBrainrotDist = nil, math.huge, math.huge
...
local function runTeleportV2SecondFloor(targetPart, hum, hrp)
    ...
    if not chosenPoint then return false, "no TP point found" end
    ...
    if not hrp or not hrp.Parent then return false, "hrp lost during jump" end
    ...
    if not hrp or not hrp.Parent then return false, "hrp lost after MED tp" end

    local reachedMed = waitUntilHeartbeat(function()
        return hrp and hrp.Parent and flatDistance(hrp.Position, medPoint.pos) <= 2.5
    end, 1.5)
    if not reachedMed then return false, "failed to reach MED" end
    ...
    local reachedTP = waitUntilHeartbeat(function()
        return hrp and hrp.Parent and (hrp.Position - chosenTargetPos).Magnitude <= 2.5
    end, 3.0)
    if not reachedTP then return false, "failed to reach TP point" end
    ...
    if not stillAtChosenPoint then return false, "moved away before clone" end
    ...
    return true, chosenPoint.name
end


local function _tp2fRun(overridePos)
    local char = LocalPlayer.Character
    local hrp  = char and char:FindFirstChild("HumanoidRootPart")
    local hum  = char and char:FindFirstChildOfClass("Humanoid")
    if not (char and hrp and hum) then return end
    if not overridePos then return end

    local targetPart = findAdorneeGlobal(_G._current3fTarget)
    ...
    if carpet then hum:EquipTool(carpet); task.wait(0.02) end

    local targetPetData = _G._current3fTarget
    local exactPos = overridePos
    ...
    local ok, err = pcall(function()
        ...
        if not medPoint then error("no MED point") end

        hrp.AssemblyLinearVelocity = Vector3.zero
        if hum and hum.Parent then hum:ChangeState(Enum.HumanoidStateType.Jumping) end
        task.wait(0.01)
        if not hrp or not hrp.Parent then error("hrp lost") end
        ...
        task.wait(0.1)
        if carpet and hum then pcall(function() hum:EquipTool(carpet) end) end

        local freshTarget = findAdorneeGlobal(_G._current3fTarget)
        local petPos = (freshTarget and freshTarget.Position) or overridePos
        ...
        if verticalDiff > 2 then
            ...
            plat.Anchored = true; plat.CanCollide = true
            plat.Transparency = 0.3; plat.Parent = Workspace
            RunService.Heartbeat:Wait()
            for i = 1, 10 do
                if not LocalPlayer:GetAttribute("Stealing") then
                    hrp = char and char:FindFirstChild("HumanoidRootPart"); if not hrp then break end
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X,0,hrp.AssemblyLinearVelocity.Z)
    ...
    if not ok then warn("[3f TP]", err) end
end
-- end tp.lua 2f
...
runAutoSnipe = function()
    ...
    -- If priority mode is enabled, scan the cache for the highest-ranked
    -- priority pet FIRST. Only fall back to highest-gen logic if none found.
    local priorityActive = (Config.StealPriority == true) or (Config.DefaultToPriority == true)
    local priorityPick
    local cache = SharedState.AllAnimalsCache
    if priorityActive and cache and type(cache) == "table" then
        local bestRank = math.huge
        for _, a in ipairs(cache) do
            if a and a.owner ~= LocalPlayer.Name and a.name then
                local aLower = tostring(a.name):lower()
                for rank, pName in ipairs(PRIORITY_LIST) do
                    if pName:lower() == aLower then
                ...
                if bestRank == 1 then break end
            end
        end
    end

    -- Find best 1f/2f pet and best 3f pet separately
    local best1f, best3f = nil, nil
    if cache and type(cache) == "table" then
        for _, a in ipairs(cache) do
            if a and a.owner ~= LocalPlayer.Name then
                local _p = findAdorneeGlobal(a)
                local _y = _p and _p.Position.Y or 0
                if _y > 25 then
                    if not best3f or (a.genValue or 0) > (best3f.genValue or 0) then
                        best3f = a
                    end
                else
                    if not best1f or (a.genValue or 0) > (best1f.genValue or 0) then
                        best1f = a
                    end
                end
            end
    ...
    -- Pick the better one
    local targetPetData
    if priorityPick then
        targetPetData = priorityPick
    else
        local gen1f = best1f and (best1f.genValue or 0) or 0
        local gen3f = best3f and (best3f.genValue or 0) or 0
        if best3f and gen3f >= gen1f then
            targetPetData = best3f
        elseif best1f then
            targetPetData = best1f
        else
    ...
    if not targetPart then
        State.isTpMoving = false; isTpMoving = State.isTpMoving; return
    end

    local exactPos = targetPart.Position

    if exactPos.Y > 25 then
        _G._current3fTarget = targetPetData
        _tp2fRun(exactPos)
        State.isTpMoving=false;isTpMoving=false
        return
    end
    ...
    if exactPos.Y > 25 then
        -- 2nd floor: go to BASES_HIGH, velocity rise, clone in, float under pet.
        local _tpStart = tick()
        local _phase = function(name) print(string.format("[TP2F] %6.3fs %s  pos=(%.1f,%.1f,%.1f)", tick()-_tpStart, name, hrp.Position.X, hrp.Position.Y, hrp.Position.Z)) end
        _phase("START          ")
        if carpet then hum:EquipTool(carpet) end
        local plotIndex = getClosestBaseIdx(exactPos)
        local targetBasePos = BASES_HIGH[plotIndex]
        local targetHeight = math.max(targetBasePos.Y, 50)
        print(string.format("[TP2F]        target plot=%d basePos=(%.1f,%.1f,%.1f)", plotIndex, targetBasePos.X, targetBasePos.Y, targetBasePos.Z))

        -- PRE-RISE CORRIDOR ESCAPE
        local curX = hrp.Position.X
        local targetColumnX = targetBasePos.X
        local inMiddle = curX > -465 and curX < -355
        if inMiddle then
            _phase("CORRIDOR-ESC IN ")
            local corridorX = (targetColumnX > -410) and -301 or -519
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.AssemblyAngularVelocity = Vector3.zero
            -- Lift to Y=30 first so we cross OVER the shop/Trade Machine/Cyber Craft
            -- structures instead of through them. At Y=30 we're above all middle-map geometry.
            local liftY = math.max(hrp.Position.Y, 30)
            hrp.CFrame = CFrame.new(corridorX, liftY, hrp.Position.Z)
            RunService.Heartbeat:Wait()
            hrp.AssemblyLinearVelocity = Vector3.zero
            char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then State.isTpMoving=false; isTpMoving=false; return end
            _phase("CORRIDOR-ESC OUT")
        end

        -- Rise with vel=400 to Y=60, then settle down to BASES_HIGH
        local jumpStart = tick()
        while hrp.Position.Y < 60 and (tick() - jumpStart) < 2 do
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 400, hrp.AssemblyLinearVelocity.Z)
            RunService.Heartbeat:Wait()
            char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then State.isTpMoving=false; isTpMoving=false; return end
        end
        -- Settle: 4 iterations with early-exit. The previous version had 2 redundant Heartbeat
        -- drains plus a 10-iteration unconditional loop = ~200ms of settle. Early-exit version
        -- does the same job in ~30-60ms typical.
        local _settleIters = 0
        for i = 1, 4 do
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
        ...
        print(string.format("[TP2F] %6.3fs SETTLE END      pos=(%.1f,%.1f,%.1f) iters=%d dist=%.1f", tick()-_tpStart, hrp.Position.X, hrp.Position.Y, hrp.Position.Z, _settleIters, (hrp.Position - targetBasePos).Magnitude))

        char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
        hum = char and char:FindFirstChildOfClass("Humanoid")
        if not hrp then State.isTpMoving=false; isTpMoving=false; return end

        local bestFace = FACE_TARGETS[1]; local minFaceDist = math.huge
        for _, v in ipairs(FACE_TARGETS) do
            local d = (hrp.Position - v).Magnitude
            if d < minFaceDist then minFaceDist = d; bestFace = v end
        end

        -- (dropped: task.wait(0.01) — face-turn is a client-only CFrame.lookAt, nothing to wait for)
        hrp.CFrame = CFrame.lookAt(hrp.Position, Vector3.new(bestFace.X, hrp.Position.Y, bestFace.Z))
        _phase("POST FACE-TURN  ")

        -- PRE-WALK STABILIZATION: kill velocity once, snap if drifted. Was a 5-iteration loop
        -- with 5 Heartbeat waits (~80ms wasted) — single check is enough.
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        if (hrp.Position - targetBasePos).Magnitude > 4 then
            hrp.CFrame = CFrame.new(targetBasePos)
            hrp.AssemblyLinearVelocity = Vector3.zero
            RunService.Heartbeat:Wait()
            hrp.AssemblyLinearVelocity = Vector3.zero
        ...
        -- ABORT if the snap didn't land us. If we're still far from targetBasePos, the server
        -- has rejected our CFrame and we'd be cloning from the wrong place (often own base).
        -- Better to bail now than place a clone marker where it doesn't belong.
        char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then State.isTpMoving=false; isTpMoving=false; return end
        if (hrp.Position - targetBasePos).Magnitude > 25 then
            print(string.format("[TP2F] >>> ABORT: not at target base (dist=%.1f)", (hrp.Position - targetBasePos).Magnitude))
            ShowNotification("TP", "Couldn't reach target base, aborted")
            State.isTpMoving = false; isTpMoving = State.isTpMoving
            return
        end

        -- Simple walk + wait. The previous version had walkForward(0.3) followed by a
        -- complex stillness-polling loop that waited up to 2s — that was the slowness.
        -- walkForward is async and stops itself after its duration, so a fixed wait is correct.
        walkForward(0.24)
        task.wait(0.24)
        _phase("WALK DONE       ")

        -- CRITICAL safety check: verify we're actually near the target's base before firing
        -- the clone. If anti-cheat, character respawn, or rubberband sent us back to our
        -- own base mid-flow, calling instantClone() now would place the marker AT OUR OWN
        -- BASE — and TeleportToClone would teleport us right back there afterward.
        -- That's the "TPs me to my own base" bug.
        char = LocalPlayer.Character
        hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then State.isTpMoving=false; isTpMoving=false; return end
        local distToTarget = (Vector3.new(hrp.Position.X, 0, hrp.Position.Z) - Vector3.new(targetBasePos.X, 0, targetBasePos.Z)).Magnitude
        if distToTarget > 30 then
            print(string.format("[TP2F] ABORT: too far from target base (%.1f studs) — likely got rubberbanded back to own base", distToTarget))
            State.isTpMoving=false; isTpMoving=false; return
        end

        local _posBeforeClone = hrp.Position
        instantClone(); while _G.isCloning do task.wait() end
        -- (dropped: task.wait(0.015) — instantClone already has task.wait(0.3) internally
        -- AND the char-exists wait below does the same job. Three waits for one thing.)
        _phase("CLONE RETURNED  ")

        do local _t=tick() repeat task.wait(0) until (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")) or tick()-_t>1 end

        char = LocalPlayer.Character
        hrp = char and char:FindFirstChild("HumanoidRootPart")
        hum = char and char:FindFirstChildOfClass("Humanoid")
        if not hrp then
            print("[TP2F] ABORT: no hrp after char-exists wait")
            State.isTpMoving=false; isTpMoving=false; return
        end
        local _cloneDist = (hrp.Position - _posBeforeClone).Magnitude
        print(string.format("[TP2F] %6.3fs CHAR-EXISTS OK  pos=(%.1f,%.1f,%.1f) cloneMovedBy=%.1f", tick()-_tpStart, hrp.Position.X, hrp.Position.Y, hrp.Position.Z, _cloneDist))
        if _cloneDist < 10 then print("[TP2F] >>> CLONE PROBABLY DIDN'T FIRE (char didn't move enough)") end

        if carpet then pcall(function() hum:EquipTool(carpet) end) end

        -- Float under the pet. Start the loop RIGHT AWAY — no pre-loop wait.
        local freshT = findAdorneeGlobal(targetPetData)
        local petPos3 = (freshT and freshT.Position) or exactPos
        local verticalDiff = petPos3.Y - hrp.Position.Y

        if verticalDiff > 2 then
            -- Gapped's red-platform + weld pattern. 3x1x3 Neon platform 5 below airPos,
            -- snap loop, then weld char to platform for 0.5s for guaranteed stillness while
            -- the steal prompt triggers.
            local airPos = Vector3.new(petPos3.X, petPos3.Y - 8, petPos3.Z)
            local plat = Instance.new("Part")
            ...
            -- Weld player to platform for guaranteed stillness
            char = LocalPlayer.Character
            hrp = char and char:FindFirstChild("HumanoidRootPart")
            if hrp then
            ...
            -- First snap BEFORE loop starts.
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.CFrame = CFrame.new(petPos3)
            local _pEnd = tick() + 3
            while not LocalPlayer:GetAttribute("Stealing") and tick() < _pEnd do
                hrp = char and char:FindFirstChild("HumanoidRootPart"); if not hrp then break end
                local _lv = findAdorneeGlobal(targetPetData)
                local _raw = (_lv and _lv.Position) or petPos3
                local drift = (hrp.Position - _raw).Magnitude
                if drift > 1 then
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
                    hrp.CFrame = CFrame.new(_raw)
                elseif math.abs(hrp.AssemblyLinearVelocity.Y) > 0.1 then
                    hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
                end
                task.wait()
            end
    ...
    -- 1st/2nd floor TP
    if carpet then hum:EquipTool(carpet) end
    -- (dropped: task.wait(0.01) — Heartbeat wait below gives the equip time to register)
    local plotIndex = getClosestBaseIdx(exactPos)

    local bestSpot = CLONE_POSITIONS_FLOOR[plotIndex] or CLONE_POSITIONS_FLOOR[1]

    -- PRE-TP CORRIDOR ESCAPE: same logic as 2f. If we're in the map middle (between base
    -- columns where shops live), step sideways into the target's corridor first. Prevents
    -- the big CFrame to bestSpot from being rejected by the server because it passes through
    -- shop geometry.
    local curX = hrp.Position.X
    local inMiddle = curX > -465 and curX < -355
    if inMiddle then
        local corridorX = (bestSpot.X > -410) and -301 or -519
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        -- Lift to Y=50 first so we cross OVER the shop/Trade Machine/Cyber Craft structures
        -- instead of through them. Without this lift, the X CFrame at low Y bounces off shops.
        local liftY = math.max(hrp.Position.Y, 50)
        hrp.CFrame = CFrame.new(corridorX, liftY, hrp.Position.Z)
        RunService.Heartbeat:Wait()
        hrp.AssemblyLinearVelocity = Vector3.zero
        char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
    ...
    -- Only do the anti-rubberband Heartbeat + second CFrame if the first one didn't land us.
    -- Most of the time the first CFrame lands perfectly (no rubberband), so we can skip the
    -- ~16ms Heartbeat + redundant second CFrame entirely.
    if (hrp.Position - bestSpot).Magnitude > 3 then
        RunService.Heartbeat:Wait()
        -- Re-zero velocity AFTER the Heartbeat wait — gravity/carpet physics can restore
        -- velocity during that frame, which then makes the second CFrame rubberband.
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        hrp.CFrame = CFrame.new(bestSpot)
        hrp.AssemblyLinearVelocity = Vector3.zero -- re-zero immediately after CFrame too
    end

    char = LocalPlayer.Character; hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then State.isTpMoving=false; isTpMoving=false; return end

    -- Safety: confirm we actually arrived at bestSpot. If the server rejected the CFrame
    -- (rubberband from leftover velocity, lag, etc.) we'd be proceeding from a bad position
    -- and the clone + pickup would fire from somewhere wrong, often killing us. Abort cleanly
    -- with a notification instead. Matches the 2f "Base unreachable" pattern.
    if (hrp.Position - bestSpot).Magnitude > 15 then
        State.isTpMoving = false; isTpMoving = State.isTpMoving
        ShowNotification("TP", "Base unreachable, aborted")
        return
    end
    ...
    do local _t=tick() repeat task.wait(0) until (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")) or tick()-_t>0.5 end
    char = LocalPlayer.Character
    hrp = char and char:FindFirstChild("HumanoidRootPart")
    hum = char and char:FindFirstChildOfClass("Humanoid")
    if not hrp then State.isTpMoving=false; isTpMoving=false; return end
    ...
    -- Pickup: if pet is above (2nd floor), float under it at petPos.Y - 8
    local freshTarget = findAdorneeGlobal(targetPetData)
    local petPos = (freshTarget and freshTarget.Position) or exactPos
    local verticalDiff = petPos.Y - hrp.Position.Y

    if verticalDiff > 2 then
        -- Gapped's red-platform + weld pattern. Same as 2f branch — guarantees stillness via
        -- physical weld so the steal prompt fires reliably.
        local airPos = Vector3.new(petPos.X, petPos.Y - 8, petPos.Z)
        local plat = Instance.new("Part")
        plat.Name = "XiTempPlatform"
        plat.Size = Vector3.new(3, 1, 3)
        ...
        while not LocalPlayer:GetAttribute("Stealing") and tick() < _pEnd do
            ...
            if not hrp then break end
            local _lv = findAdorneeGlobal(targetPetData)
            if _lv then airPos = Vector3.new(_lv.Position.X, _lv.Position.Y - 8, _lv.Position.Z) end
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.CFrame = CFrame.new(airPos)
            task.wait(0.02)
        end
        ...
        -- First snap BEFORE loop.
        local sitPos = Vector3.new(petPos.X, petPos.Y + 3, petPos.Z)
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.CFrame = CFrame.new(sitPos)
        local _pEnd = tick() + 3
        while not LocalPlayer:GetAttribute("Stealing") and tick() < _pEnd do
            hrp = char and char:FindFirstChild("HumanoidRootPart")
            if not hrp then break end
            local _lv = findAdorneeGlobal(targetPetData)
            local _raw = _lv and _lv.Position or petPos
            sitPos = Vector3.new(_raw.X, _raw.Y + 3, _raw.Z)
            local drift = (hrp.Position - sitPos).Magnitude
            if drift > 1 then
                hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
                hrp.CFrame = CFrame.new(sitPos)
            elseif math.abs(hrp.AssemblyLinearVelocity.Y) > 0.1 then
...
do
    ...
    actionsTitle.Text = "YOU & BLOOM HUB"
...
task.spawn(function()
    ...
    local HTheme = {
        Background = Color3.fromRGB(8, 10, 18),         -- match main Theme.Background
        Surface    = Color3.fromRGB(14, 16, 28),        -- match main Theme.Surface
        Accent1    = Color3.fromRGB(80, 255, 140),      -- main Theme.Accent1 (neon green)
        Accent2    = Color3.fromRGB(40, 200, 100),      -- main Theme.Accent2 (secondary green)
        White      = Color3.fromRGB(245, 250, 255),     -- match main Theme.TextPrimary
        Gray       = Color3.fromRGB(160, 175, 200),     -- match main Theme.TextSecondary
    }
    ...
    main.BackgroundColor3 = HTheme.Background
    ...
    bgGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, HTheme.Background),
        ColorSequenceKeypoint.new(1, HTheme.Surface)
    }
    bgGradient.Rotation = 45

    local stroke = Instance.new("UIStroke", main)
    stroke.Color = HTheme.Accent2
    stroke.Thickness = 1.5
    ...
    task.spawn(function()
        while stroke.Parent do
            strokeGrad.Rotation = (strokeGrad.Rotation + 0.3) % 360
            task.wait(0.05)
        end
    end)
    ...
    -- Logo badge (rounded square with custom image) on the far left
    local logoBadge = Instance.new("Frame", topBar)
    logoBadge.Name = "LogoBadge"
    logoBadge.Size = UDim2.new(0, 34*SCALE, 0, 34*SCALE)
    logoBadge.Position = UDim2.new(0, 10*SCALE, 0.5, 0)
    logoBadge.AnchorPoint = Vector2.new(0, 0.5)
    logoBadge.BackgroundColor3 = Color3.fromRGB(15, 25, 15) -- dark backdrop behind the image
    ...
    logoBadgeStroke.Color = HTheme.Accent1 -- green border keeps the theme
    logoBadgeStroke.Thickness = 1.5
    logoBadgeStroke.Transparency = 0.4

    -- >>> PASTE YOUR IMAGE ID BELOW <<<
    -- Upload the image to Roblox (Avatar Shop > Create > Decals) then put the
    -- numeric asset id here, e.g. "rbxassetid://123456789".
    local LOGO_IMAGE_ID = "rbxthumb://type=Asset&id=118617668793203&w=150&h=150"

    local logoImage = Instance.new("ImageLabel", logoBadge)
    ...
    brandTextLayout.Padding = UDim.new(0, 0)

    -- Main title row: "YOU & BLOOM HUB | MOGGED" with RichText so the
    -- divider and link can have their own colors like in the reference.
    local title = Instance.new("TextLabel", brandText)
    title.RichText = true
    title.Text = string.format(
        "<b>YOU & BLOOM HUB</b>  <font color='rgb(120,130,150)'>|</font>  <font color='rgb(%d,%d,%d)'><b>MOGGED</b></font>",
        math.floor(HTheme.Accent1.R*255),
        math.floor(HTheme.Accent1.G*255),
        math.floor(HTheme.Accent1.B*255)
    )
    ...
    -- Animated green streak that sweeps across the title.
    -- White at the edges (so RichText colors show through unchanged) with a
    -- narrow neon-green band in the middle that we slide left-to-right.
    local streakGrad = Instance.new("UIGradient", title)
    streakGrad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0,    Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(0.42, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(0.50, HTheme.Accent1),
        ColorSequenceKeypoint.new(0.58, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(1,    Color3.fromRGB(255, 255, 255)),
    }
    streakGrad.Rotation = 20
    streakGrad.Offset = Vector2.new(-1, 0)
    ...
    -- Subtitle: faded credit / version line
    local brandCredit = Instance.new("TextLabel", brandText)
    brandCredit.Text = "by you & bloom · v2.0"
    brandCredit.Font = Enum.Font.GothamMedium
    ...
    if privateBuild then
        ...
        privateTag.Size = UDim2.new(0, 0, 0, 12*SCALE)
        privateTag.Position = UDim2.new(0, 54*SCALE, 0.5, 14*SCALE)
        privateTag.AnchorPoint = Vector2.new(0, 0.5)
        privateTag.TextTransparency = 0.1
        privateTag.TextXAlignment = Enum.TextXAlignment.Left
    end

    -- Subtle pulse on the logo badge stroke for a touch of life
    task.spawn(function()
        while logoBadgeStroke.Parent do
...
-- Steal ESP: shows "STEALING <pet>" above the thief's home plot sign (so you can see which
-- player's base on the map is currently engaged in stealing). Adornee is the PlotSign owned
-- BY the thief — same pattern as BULLYHUB. Self-contained, no external deps.
task.spawn(function()
    ...
    if Config.ShowStealingESP == nil then Config.ShowStealingESP = true end

    local espGui = Instance.new("ScreenGui")
    espGui.Name = "XiStealingESP"
    espGui.ResetOnSpawn = false
    espGui.Enabled = Config.ShowStealingESP ~= false
    espGui.Parent = PlayerGui

    -- userId -> { bill = BillboardGui, adornee = part }
    local active = {}

    local function clearOne(uid)
        local r = active[uid]
        if r and r.bill and r.bill.Parent then r.bill:Destroy() end
        active[uid] = nil
    end

    local function clearAll()
        for uid in pairs(active) do clearOne(uid) end
    end

    Players.PlayerRemoving:Connect(function(plr) clearOne(plr.UserId) end)

    -- Find the plot owned by the given player (by matching their name on the PlotSign).
    ...
    local function getPlotSignPart(plot)
    ...
    local function buildBillboard(adorneePart, plr, petName)
        local bb = Instance.new("BillboardGui")
        bb.Name = "XiStealingESP_" .. plr.Name
        ...
        title.Text = (plr.DisplayName or plr.Name) .. " · STEALING"
        ...
        caption.Position = UDim2.new(0, 6, 0, 32)
        caption.BackgroundTransparency = 1
        caption.Font = Enum.Font.GothamMedium
        caption.TextSize = 11
        caption.TextColor3 = Color3.fromRGB(170, 170, 170)
        caption.TextXAlignment = Enum.TextXAlignment.Center
        caption.Text = "Stealing item:"
        caption.Parent = root

        -- Pet/item name — the big prominent line
        local sub = Instance.new("TextLabel")
        sub.Name = "Sub"
        sub.Size = UDim2.new(1, -12, 0, 44)
        sub.Position = UDim2.new(0, 6, 0, 50)
        ...
        sub.Text = (petName ~= "" and petName) or "?"
        sub.Parent = root
    ...
    while true do
        task.wait(0.4)
        local enabled = Config.ShowStealingESP ~= false
        ...
        if not enabled then
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr:GetAttribute("Stealing") then
                    ...
                    petName = petName and tostring(petName) or ""

                    local plot = findPlotForPlayer(plr)
                    local part = getPlotSignPart(plot)
                    if part and part:IsDescendantOf(Workspace) then
                        seen[plr.UserId] = true

                        local row = active[plr.UserId]
                        -- Rebuild if adornee dead/changed
                        if row and (row.adornee ~= part or not row.bill or not row.bill.Parent) then
                            clearOne(plr.UserId); row = nil
                        end

                        if not row then
                            local bb = buildBillboard(part, plr, petName)
                            if bb then active[plr.UserId] = { bill = bb, adornee = part } end
                        else
                            local sub = row.bill:FindFirstChild("Sub", true)
                            local newText = (petName ~= "" and petName) or "?"
                            if sub and sub.Text ~= newText then sub.Text = newText end
                        end
                    end
                end
            end
            ...
            for uid in pairs(active) do
                if not seen[uid] then table.insert(stale, uid) end
            end
            for _, uid in ipairs(stale) do clearOne(uid) end
        end
    end
end)

-- ==================================================================
-- Auto Pickup — hub-styled draggable panel
-- Collects all brainrots on YOUR plot, then disconnects you.
-- Position is saved to Config.Positions.AutoPickup so it persists
-- between sessions.
-- ==================================================================
task.spawn(function()
    if PlayerGui:FindFirstChild("XiAutoPickup") then
        PlayerGui.XiAutoPickup:Destroy()
    end

    -- Synchronizer (used to find the local player's plot)
    local Synchronizer
    do
        local pkg = ReplicatedStorage:WaitForChild("Packages", 30)
        local syncMod = pkg and pkg:WaitForChild("Synchronizer", 30)
        if syncMod then
            local ok, mod = pcall(require, syncMod)
            if ok then Synchronizer = mod end
        end
    end
    ...
    local function getMyPlot()
        ...
        if Synchronizer then
            for _, plot in ipairs(plots:GetChildren()) do
                ...
                if ok and channel then
                    local ownerOk, owner = pcall(function() return channel:Get("Owner") end)
                    if ownerOk and owner then
                        if (typeof(owner) == "Instance" and owner == LocalPlayer)
                        or (typeof(owner) == "table" and owner.UserId == LocalPlayer.UserId) then
                            return plot
                        end
                    end
                end
        ...
        local dn = (LocalPlayer.DisplayName or ""):lower()
        local un = (LocalPlayer.Name or ""):lower()
        for _, plot in ipairs(plots:GetChildren()) do
            local sign = plot:FindFirstChild("PlotSign")
            if sign then
                ...
                if lbl and lbl.Text then
                    ...
                    if (dn ~= "" and t:find(dn, 1, true))
                    or (un ~= "" and t:find(un, 1, true)) then
                        return plot
                    end
                end
            end
    ...
    local function getOccupiedSlots(plot)
        local slots = {}
        if not plot or not Synchronizer then return slots end
        local ok, ch = pcall(function() return Synchronizer:Get(plot.Name) end)
        if not ok or not ch then return slots end
        local listOk, list = pcall(function() return ch:Get("AnimalList") end)
        if not listOk or type(list) ~= "table" then return slots end
    ...
    local function findPromptForSlot(plot, slotKey)
        local podiums = plot:FindFirstChild("AnimalPodiums")
        if not podiums then return nil end
        local podium = podiums:FindFirstChild(slotKey)
    ...
    local function forceDisconnect()
        pcall(function()
            game:GetService("TeleportService"):Teleport(0, LocalPlayer)
        end)
        local env = getfenv()
        for _, name in ipairs({"shutdown", "exitgame", "queue_on_teleport"}) do
            local fn = rawget(env, name) or rawget(_G, name)
            if type(fn) == "function" then pcall(fn) end
        end
        pcall(function() game:Shutdown() end)
        pcall(function() LocalPlayer:Kick("[Auto Pickup] Disconnecting.") end)
        task.spawn(function()
            task.wait(0.3)
            while true do
                pcall(function() game:Shutdown() end)
    ...
    local function runAutoPickup(setBack)
        if autoPickupRunning then return end
        autoPickupRunning = true
        task.spawn(function()
            local plot = getMyPlot()
            if not plot then
                if ShowNotification then ShowNotification("AUTO PICKUP", "Plot not found") end
                autoPickupRunning = false
                if setBack then setBack(false) end
                return
            end
            local slots = getOccupiedSlots(plot)
            if #slots == 0 then
                if ShowNotification then ShowNotification("AUTO PICKUP", "No brainrots on plot") end
                autoPickupRunning = false
                if setBack then setBack(false) end
                return
            end
            if ShowNotification then
                ShowNotification("AUTO PICKUP", string.format("Collecting %d slot(s)...", #slots))
            end
            for _, slotKey in ipairs(slots) do
                local prompt = findPromptForSlot(plot, slotKey)
                if prompt then firePrompt(prompt) end
            end
            task.wait(0.5)
            if ShowNotification then ShowNotification("AUTO PICKUP", "Disconnecting...") end
            forceDisconnect()
            autoPickupRunning = false
        end)
    end

    -- ---------- UI (hub-styled, mirrors JobJoiner / Steal panels) ----------
    local SCALE = IS_MOBILE and 0.65 or 1

    local gui = Instance.new("ScreenGui")
    gui.Name = "XiAutoPickup"
    gui.ResetOnSpawn = false
    ...
    local main = Instance.new("Frame")
    main.Name = "Main"
    main.Size = UDim2.new(0, 230 * SCALE, 0, 56 * SCALE)
    local savedPos = (Config.Positions and Config.Positions.AutoPickup) or {X = 0.78, Y = 0.12}
    main.Position = UDim2.new(savedPos.X, 0, savedPos.Y, 0)
    ...
    -- Subtle gradient background (matches JobJoiner panel feel)
    local bgGrad = Instance.new("UIGradient", main)
    bgGrad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Theme.Surface),
        ColorSequenceKeypoint.new(1, Theme.SurfaceHighlight),
    }
    bgGrad.Rotation = 45
    ...
    -- Make the whole card draggable + persist position via the hub helper
    MakeDraggable(main, main, "AutoPickup")

    -- Accent bar on the left (same idiom as section headers in the main hub)
    local accentBar = Instance.new("Frame", main)
    accentBar.Size = UDim2.new(0, 3, 0, 22 * SCALE)
    accentBar.Position = UDim2.new(0, 10 * SCALE, 0.5, -11 * SCALE)
    ...
    title.Size = UDim2.new(1, -110 * SCALE, 0, 16 * SCALE)
    title.Position = UDim2.new(0, 22 * SCALE, 0, 10 * SCALE)
    title.BackgroundTransparency = 1
    title.Font = Enum.Font.GothamBlack
    title.TextSize = 12 * SCALE
    title.TextColor3 = Theme.TextPrimary
    title.TextXAlignment = Enum.TextXAlignment.Left
    title.Text = "AUTO PICKUP"

    local subtitle = Instance.new("TextLabel", main)
    subtitle.Size = UDim2.new(1, -110 * SCALE, 0, 14 * SCALE)
    subtitle.Position = UDim2.new(0, 22 * SCALE, 0, 28 * SCALE)
    subtitle.BackgroundTransparency = 1
    subtitle.Font = Enum.Font.Gotham
    subtitle.TextSize = 10 * SCALE
    subtitle.TextColor3 = Theme.TextSecondary
    subtitle.TextXAlignment = Enum.TextXAlignment.Left
    subtitle.Text = "Collect plot + leave"

    -- Toggle switch (uses the hub's CreateToggleSwitch helper)
    local toggleHost = Instance.new("Frame", main)
    toggleHost.Size = UDim2.new(0, 50 * SCALE, 0, 24 * SCALE)
    toggleHost.AnchorPoint = Vector2.new(1, 0.5)
    toggleHost.Position = UDim2.new(1, -12 * SCALE, 0.5, 0)
    toggleHost.BackgroundTransparency = 1

    local toggle = CreateToggleSwitch(toggleHost, false, function(newState, set)
        if newState then
            set(true)
            runAutoPickup(set)
        else
    ...
    -- Override default position so it sits centered inside our small host frame
    if toggle and toggle.Container then
        toggle.Container.AnchorPoint = Vector2.new(0.5, 0.5)
        toggle.Container.Position = UDim2.new(0.5, 0, 0.5, 0)
    end
end)
